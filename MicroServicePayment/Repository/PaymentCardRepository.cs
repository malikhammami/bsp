...
public class PaymentCardRepository : IPaymentCardRepository
{
    ...
    public Paymentcard? GetPaymentCardByCardNumber(string cardNumber)
    {
        return _dbContext.Paymentcards.SingleOrDefault(pc => pc.Cardnumber == cardNumber);
    }

    public void UpdatePaymentCard(Paymentcard paymentCard)
    {
        _dbContext.Paymentcards.Update(paymentCard);
